...
**T1109 航班预定系统 //python3**

【思考】显然，题设的数据量是卡了暴力过的可能性的，必然TLE。自然的思路是考虑对所有区间操作信息做预处理，我的想法是排序并统计重叠片段和，但显然，不论怎么操作，最坏情况都会趋向于n*m。

【借鉴】对一整个区间操作，只需要统计一头一尾就行。具体到实际技巧，是差分数组。实际上前缀和我们经常做，但差分数组的技巧还是第一次碰到。如此操作，复杂度仅为O(n+m)
...
**T1894 找到需要补充粉笔的学生编号 //python3**

【注意】这题如果数组很长，应该用前缀和+二分查找

【思路】但是这题，，，你怎么写至少也是O(n)，那前缀和+二分的意义真就不是很大，而且他和是先求的，也就讲除非是卡常数的题，不然不可能会出现超时的情况，，，，
...
【注意】统计个数的时候注意避开读取None的值，也注意range取值的问题，同时考虑的时候需要细心。
...
还凑合，我现在已经完全想明白为什么最后一题贪心的思路不对了（比赛的时候已经意识到了，其实意识到的时候就该立刻换思路），这题贪心是不行的，要么状态压缩+暴搜，要么就高维动规，其实也没有特别简明的解决思路。
...
【思路】唔，这题现在看起来直接用哈希表会快很多

**1818 绝对差值和**

【思路】就是要在序列2中，对每一个元素，找序列1里面与其绝对值差别最小的。结合数据范围，容易想到，预处理方式是排序，查询方式为二分查找。

**1819 序列中不同最大公约数的数目**

【思路】其实基本上算是做出来了，之前做并查集的时候做过大量这样的题目，所以拿到手就知道应该枚举因数。

但是我犯了个2！比如讲，如果要求2是最大公约数，2×4，2×8，2×12都入队了，也就是有了8、16和24，这样最大公约数其实不是2，而是4。实际上就要求，我们要想枚举的是最大公约数，另一个因数之间必须互质，**但是，并不是要求两两互质！2，4，5也是可以的，比如4、8、10的最大公约数就是2，剩下的因子的最大公约数是1就行了，没要求你两两互质啊！实际上有两个因子互质就已经满足条件了！！！**，
