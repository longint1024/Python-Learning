...
a = b[:] #如此可以极其简单地实现列表的深拷贝，且和不需要考虑a、b本身大小

a.reverse() #过程，反转列表a
a[1:3].reverse() #这是不会翻转a[1:3]的，因为a[1:3]实际上是另一个列表，只有对a的值的拷贝，与真正的a的那部分并不共用一篇地址
a[1:3] = a[2:0:-1] #这样可以翻转a，如果对这份拷贝做了修改，会改变a
a[0:2] = a[1:-1:-1] #这样又不行了，因为-1代表的是n-1这个位置，右边这个拷贝实际上为空。
```
...
双向队列是可以在O(1)时间从队列的两端删除元素的特殊队列。经典的是双向队列解决滑动窗口最大值问题。滑动窗口连续和最大可以用前缀和解决，滑动窗口内最大值可以在线性时间复杂度内实现，实现方式有双向队列和动态规划。

针对滑动窗口最大值问题，我们可以设计一种特殊的数据结构来解决：单调队列。因为滑动窗口最大值具有以下性质：如果一个元素后入队且更大，那么在它之前入队且比它更小的元素都可以不必保留在滑动窗口内！因为当新的更大的元素入队后，直到这些“低等”元素被移出滑动窗口的生命周期中，它们没有成为最大值的可能。这就是后浪拍前浪了：滑动窗口最大值问题可以优化的原因就是，优秀的“后浪”会把前浪拍死在沙滩上，这些前浪大可不必留在队列中自取其辱参与比较浪费时间。

所以我们直接末尾淘汰，按入队顺序排个队，后进来的如果更加优秀，就可以依次把它前面都踢走（也许可以都踢完！），直到把前浪全拍死了，或者有更高的前浪为止。由此不难发现，单调队列如其名称，其中元素按下表顺序排列（前浪在前，后浪在后），且前浪必须比后浪高。

如何解决从窗口前端滑出去的元素的出队问题呢？这个也容易，因为我们需要的最大值可以在入队操作中记录，所以在队列里面待着的只需要是index即可，由于队首一定是最老资格的，所以判断一下队首需不需要出去就行了。这也是我们需要用双端队列的原因，左端出也得是O(1)的。
...
注：这上面的是更新了一个版本的并查集，之前的版本连通块大小有点问题。
...
很多题需要用筛法找质因数并有特殊的处理技巧，并不是看到公因数就无脑上GCD的。比如T952按公因数计算最大组件和T1627带阈值的图连通性。



### 打表与bisect

bisect是个python的插入排序库，功能强大不得不提。

这道题顺带强调了打表的思想，不要看到幂和模就无脑上高幂模，有时候打表还来得快一点。
